...
TEST_CASE("SkipList::traverse::size == 1 after three elements were added with the same key", "[weight=5][part=1]") {
    ...
    vector<int> result = list.traverse();
    removeSentinels(result);
    // HSLAPixel ret = list.search(42);
    // HSLAPixel ret2 = list.search(43);
    // HSLAPixel ret3 = list.search(195);
    // REQUIRE(ret == HSLAPixel(180,1,0.5));
    // REQUIRE(ret2 == HSLAPixel(210,1,0.5));
    // REQUIRE(ret3 == HSLAPixel(0,0,0,0.5));
